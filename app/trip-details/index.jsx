...
const DESTINATION_IMAGES = {
  ...
  amsterdam: "https://upload.wikimedia.org/wikipedia/commons/thumb/b/be/AmsterdamNetwork.jpg/800px-AmsterdamNetwork.jpg",
  venice: "https://upload.wikimedia.org/wikipedia/commons/thumb/5/5d/Venice_Canal_Grande.jpg/800px-Venice_Canal_Grande.jpg",
};
...
export default function TripDetails() {
  ...
  const contentMaxWidth = isWeb ? Math.min(screenWidth, 900) : screenWidth;
  ...
  const hotels = travelPlan.hotels || travelPlan.hotelOptions || plan?.hotels || plan?.hotelOptions || [];
  const itinerary = travelPlan.itinerary || travelPlan.dailyItinerary || plan?.itinerary || plan?.dailyItinerary || {};
  const locationName = tripData?.locationInfo?.name || "Your Trip";
  const photoUri = getDestinationImage(locationName, tripData?.locationInfo?.photoRef);
  const hasItinerary =
    (typeof itinerary === "object" && !Array.isArray(itinerary) && Object.keys(itinerary).length > 0) ||
    (Array.isArray(itinerary) && itinerary.length > 0);

  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.background }}>
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={{ maxWidth: isWeb ? 1200 : undefined, width: "100%", alignSelf: "center" }}>
          ...
          <TouchableOpacity style={styles.deleteTopButton} onPress={handleDelete} disabled={deleting}>
        ...
        <View style={[styles.contentWrapper, { backgroundColor: theme.colors.background }]}>
          <View style={{ maxWidth: contentMaxWidth, width: "100%", alignSelf: "center", paddingHorizontal: 20 }}>
            ...
            <Text style={[styles.metaText, { color: theme.colors.textSecondary }]}>
              ...
              {tripData?.travelerInfo?.title ? ` \u2022 ${tripData.travelerInfo.title}` : ""}
            </Text>
            {tripData?.startDate && (
            ...
            <View style={styles.chipsRow}>
              {tripData?.budgetInfo?.title && (
                <View style={[styles.chip, { backgroundColor: theme.colors.primary + "20" }]}>
                  ...
                  <Text style={[styles.chipText, { color: theme.colors.primary }]}>{tripData.budgetInfo.title}</Text>
                </View>
              )}
              {tripData?.travelerInfo?.title && (
                <View style={[styles.chip, { backgroundColor: theme.colors.primary + "20" }]}>
                  <Ionicons name="people-outline" size={13} color={theme.colors.primary} />
                  <Text style={[styles.chipText, { color: theme.colors.primary }]}>{tripData.travelerInfo.title}</Text>
                </View>
              )}
              {tripData?.totalDays && (
                <View style={[styles.chip, { backgroundColor: theme.colors.primary + "20" }]}>
                  <Ionicons name="calendar-outline" size={13} color={theme.colors.primary} />
                  <Text style={[styles.chipText, { color: theme.colors.primary }]}>{tripData.totalDays} days</Text>
                </View>
              )}
            </View>
            ...
            {hotels.length > 0 && (
              <View style={styles.section}>
                <View style={styles.sectionTitleRow}>
                  ...
                  <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Hotel Recommendations</Text>
                </View>
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                  {hotels.map((hotel, index) => (
                    <View key={index} style={[styles.hotelCard, { backgroundColor: theme.colors.card }]}>
                      <Image source={{ uri: HOTEL_IMAGES[index % HOTEL_IMAGES.length] }} style={styles.hotelImage} resizeMode="cover" />
                      <View style={styles.hotelInfo}>
                        <Text style={[styles.hotelName, { color: theme.colors.text }]} numberOfLines={2}>
                          {hotel.hotelName || hotel.name}
                        </Text>
                        <View style={styles.hotelMetaRow}>
                          <Ionicons name="cash-outline" size={13} color={theme.colors.textSecondary} />
                          <Text style={[styles.hotelMetaText, { color: theme.colors.textSecondary }]}>
                            {hotel.price || hotel.pricePerNight || "N/A"}
                          </Text>
                        </View>
                        <View style={styles.hotelMetaRow}>
                          <Ionicons name="star" size={13} color="#FFB300" />
                          <Text style={[styles.hotelMetaText, { color: theme.colors.text }]}>{hotel.rating || "N/A"}</Text>
                        </View>
                        {hotel.description && (
                          <Text style={[styles.hotelDesc, { color: theme.colors.textSecondary }]} numberOfLines={2}>
                            {hotel.description}
                          </Text>
                        )}
                      </View>
            ...
            <View style={styles.section}>
              <View style={styles.sectionTitleRow}>
                ...
                <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Daily Itinerary</Text>
              </View>
              {!hasItinerary ? (
                ...
                Object.keys(itinerary).sort().map((dayKey, index) => {
                  ...
                  return (
                    <View key={dayKey} style={[styles.dayContainer, { backgroundColor: theme.colors.card }]}>
                      <Text style={[styles.dayTitle, { color: theme.colors.primary }]}>
                        Day {index + 1}{day.theme ? `: ${day.theme}` : day.title ? `: ${day.title}` : ""}
                      </Text>
                      {day.bestTime && (
                        <View style={styles.bestTimeRow}>
                          <Ionicons name="time-outline" size={12} color={theme.colors.textSecondary} />
                          <Text style={[styles.bestTime, { color: theme.colors.textSecondary }]}>{day.bestTime}</Text>
                        </View>
                      )}
                      {places.length > 0 ? (
                        places.map((place, idx) => <PlaceCard key={idx} place={place} idx={index * 3 + idx} theme={theme} />)
                      ) : (
                ...
                itinerary.map((day, index) => {
                  const places = day.plan || day.places || day.activities || [];
                  return (
                    <View key={index} style={[styles.dayContainer, { backgroundColor: theme.colors.card }]}>
                      <Text style={[styles.dayTitle, { color: theme.colors.primary }]}>
                        Day {index + 1}{day.theme ? `: ${day.theme}` : ""}
                      </Text>
                      {places.map((place, idx) => <PlaceCard key={idx} place={place} idx={index * 3 + idx} theme={theme} />)}
                    </View>
                  );
            ...
            <TouchableOpacity onPress={handleDelete} disabled={deleting} style={styles.deleteBottomBtn}>
              {deleting ? (
...
function PlaceCard({ place, idx, theme }) {
  ...
  const imageUri = place.placeImageUrl || place.imageUrl || place.image || PLACE_IMAGES[idx % PLACE_IMAGES.length];

  return (
    <View style={[styles.placeCard, { backgroundColor: theme.colors.background }]}>
      <Image source={{ uri: imageUri }} style={styles.placeImage} resizeMode="cover" />
      <View style={styles.placeInfo}>
        <Text style={[styles.placeName, { color: theme.colors.text }]} numberOfLines={2}>{name}</Text>
        {details && (
          <Text style={[styles.placeDetails, { color: theme.colors.textSecondary }]} numberOfLines={3}>{details}</Text>
        )}
        {time && (
          <View style={styles.placeMetaRow}>
            <Ionicons name="time-outline" size={12} color={theme.colors.primary} />
            <Text style={[styles.placeTime, { color: theme.colors.primary }]}>{time}</Text>
          </View>
        )}
        {ticket && (
          <View style={styles.placeMetaRow}>
            <Ionicons name="ticket-outline" size={12} color={theme.colors.textSecondary} />
            <Text style={[styles.placeTicket, { color: theme.colors.textSecondary }]}>{ticket}</Text>
          </View>
        )}
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  backButton: { position: "absolute", top: 50, left: 16, backgroundColor: "rgba(0,0,0,0.55)", borderRadius: 20, padding: 8 },
  deleteTopButton: { position: "absolute", top: 50, right: 16, backgroundColor: "rgba(220,38,38,0.85)", borderRadius: 20, padding: 8 },
  contentWrapper: { marginTop: -24, borderTopLeftRadius: 30, borderTopRightRadius: 30, paddingTop: 20 },
  locationTitle: { fontSize: 28, fontWeight: "700", marginBottom: 6 },
  metaText: { fontSize: 16, marginBottom: 6 },
  ...
  hotelCard: { width: 220, marginRight: 14, borderRadius: 16, overflow: "hidden", shadowColor: "#000", shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.08, shadowRadius: 6, elevation: 2 },
  hotelImage: { width: "100%", height: 130 },
  hotelInfo: { padding: 12 },
  hotelName: { fontSize: 15, fontWeight: "700" },
  hotelMetaRow: { flexDirection: "row", alignItems: "center", gap: 5, marginTop: 5 },
  hotelMetaText: { fontSize: 13 },
  hotelDesc: { fontSize: 11, marginTop: 6, lineHeight: 15 },
  dayContainer: { borderRadius: 18, padding: 18, marginBottom: 14, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 5, elevation: 1 },
  dayTitle: { fontSize: 17, fontWeight: "700", marginBottom: 6 },
  bestTimeRow: { flexDirection: "row", alignItems: "center", gap: 5, marginBottom: 10 },
  bestTime: { fontSize: 12 },
  placeCard: { flexDirection: "row", padding: 10, borderRadius: 14, marginTop: 10 },
  ...
  placeDetails: { fontSize: 12, marginTop: 4, lineHeight: 16 },
  placeMetaRow: { flexDirection: "row", alignItems: "center", gap: 5, marginTop: 5 },
  placeTime: { fontSize: 12 },
  placeTicket: { fontSize: 12 },
  noItineraryBox: { alignItems: "center", paddingVertical: 32, gap: 12 },
  noDataText: { fontSize: 14, color: "#999", textAlign: "center", marginTop: 8, lineHeight: 22 },
  deleteBottomBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, padding: 16, borderRadius: 14, borderWidth: 1.5, borderColor: "#dc2626", backgroundColor: "#fff5f5", marginTop: 24 },
